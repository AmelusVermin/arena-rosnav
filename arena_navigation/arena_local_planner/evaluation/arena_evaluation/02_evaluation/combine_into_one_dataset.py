...
def extend_dataset(data, file:str):
    ...
    _new_data['Planner'] = f"Agent {_file_meta_infos[2]}"
    _new_data['Robot'] = "turtlebot3"
    if 'qual' in _file_meta_infos:
        _new_data['World'] = _file_meta_infos[5]
    else:
        _new_data['World'] = _file_meta_infos[3]
        
    #_new_data['Planner'], _new_data['Robot'], _new_data['World'] = _file_meta_infos[1:-1]
    _new_data['Dyn_obs'] = int(_file_meta_infos[-1].split(".")[0]) #int(re.search(r'\d+', _file_meta_infos[-1]).group())
